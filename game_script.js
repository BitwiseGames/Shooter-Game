...
var enemies = [];
const MAX_ENEMIES = 5;
var enemyBaseSpeed = 1; // the slowest enemies move at 1px per frame
...
function startGame() {
  ...
  // create the debris
  for (var i = 0; i < MAX_DEBRIS * 2; i++) { // multiply by 2 b/c there's debris on left and right
    if (Math.random() < 0.75) { // leave some holes in the debris
...
function createEnemy() {
  ...
  if (randomNumber > 0 && randomNumber <= 0.25) {
    ...
    g.drawCircle(0, 0, 30);
    enemy.radius = 30;
    enemy.speed = enemyBaseSpeed + 2;
    enemy.health = 1;
    enemy.scoreGiven = 10;
  } else if (randomNumber > 0.25 && randomNumber <= 0.5) {
    g.beginFill("rgba(25,25,25,0.5"); // he's almost invisible!
    g.drawCircle(0, 0, 40);
    enemy.radius = 40;
    ...
    for (var j = 0; j < enemies.length; j++) {
      ...
      if (circleCollisions(bx, by, bulletSize, ex, ey, enemies[i].radius)) {
        ...
        if (enemies[j].health <= 0) {
          ...
          // the enemies move faster, and the player can shoot more bullets, every 1000 points
          if (gameScore % 1000 === 0){
            enemyBaseSpeed += 2;
            maxCoolDown --;
          }
...
function gameOver() {
  ...
  hitBox.graphics.drawRect(0, 0, restartText.getMeasuredWidth(), restartText.getMeasuredHeight());
  restartText.hitArea = hitBox;
...
function pressKey(e) {
  switch (e.keyCode) {
    ...
    case 68: rightKey = true; break; // 68 is D key
    case 83: downKey = true; break; // 83 is S key
    case 87: upKey = true; break; // 67 is W key
...
function releaseKey(e) {
  switch (e.keyCode) {
    case 65: leftKey = false; break;
    case 68: rightKey = false; break;
    case 83: downKey = false; break;
    case 87: upKey = false; break;
